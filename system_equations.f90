...
subroutine vector_field(t,y,dy,tau,Delta,I_ext,eta,tau_a,alpha,weight)
...
double precision :: r_in(1)
double precision, intent(inout) :: dy(4)
double precision, intent(in) :: tau
double precision, intent(in) :: Delta
double precision, intent(in) :: I_ext(1)
...
dy(1:1) = (Delta/(pi*tau) + 2.0*r*v)/tau
dy(2:2) = (I_ext - a + eta - pi**2*r**2*tau**2 + r_in*tau + v**2)/tau
dy(3:3) = x/tau_a
dy(4:4) = -a/tau_a + alpha*r - 2.0*x/tau_a
