        ...
        for s in input_strengths:
            for gE in coupling_strengths_E:
                for gI in coupling_strengths_I:
                    ...
                    filename_rates = f"rates_gE{gE}gI{gI}_{cortex_type}_Iduration{d}_{input_type}Istrength{s}_Ionset{input_onset}_tauVisual_{thalamus_source}_thalEI0.csv"
                    rates_df = pd.read_csv(os.path.join(output_dir, filename_rates))
                    
                    # read in potentials in data matrix (datapoints x populations)
                    filename_potentials = f"potentials_gE{gE}gI{gI}_{cortex_type}_Iduration{d}_{input_type}Istrength{s}_Ionset{input_onset}_tauVisual_{thalamus_source}_thalEI0.csv"
                    ...
                    if load_full_potentials:
                        ...
                        filename = f"full_potentials_gE{gE}gI{gI}_{cortex_type}_Iduration{d}_{input_type}Istrength{s}_Ionset{input_onset}_tauVisual_{thalamus_source}_thalEI0.csv"
