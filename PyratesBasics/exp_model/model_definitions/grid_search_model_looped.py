...
gI_thal = 200.0
# %%
for backgrndI in backgrndI_strengths:
    ...
    model_gthal.update_var(node_vars={'BACKGROUND/RPO_bI/bI': backgrndI})
    ...
    for key in results_map.index:
        ...
        fig.suptitle(f"backgrndI_{backgrndI} - Potentials: {full_title}", fontsize=16, fontweight='bold')
        plt.tight_layout(rect=[0, 0, 1, 0.92])

        # --- Safe filename ---
        safe_values = [f"backgrndI_{backgrndI}_{par_name}_{results_map.loc[key, par_name]:.4f}" for par_name in par_names]
        filename = f"backgrndI_{backgrndI}_{'_'.join(safe_values)}.png"
        figs_path = os.path.join(figs_folder, filename)
        fig.savefig(figs_path, dpi=300, bbox_inches="tight")
        # plt.close(fig)
    # %%