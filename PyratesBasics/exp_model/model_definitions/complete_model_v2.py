...
gE_thal_definition = OperatorTemplate(
    name="gE_thal_definition",
    equations="gE_thalC = gE_thal_input",
    variables={"gE_thalC": "output", "gE_thal_input": f"input({float(gE_thal)})"}
)
gI_thal_definition = OperatorTemplate(
    name="gI_thal_definition",
    equations="gI_thalC = gI_thal_input",
    variables={"gI_thalC": "output", "gI_thal_input": f"input({float(gI_thal)})"}
)
...
connectivityE_thal = OperatorTemplate(name="connectivityE_thal", 
                           equations= "m_out = (gE_thal*bEI_thal)*m_outC",
                           variables={
                            "m_out": "output", 
                            #"g_thal": g_thal,
                            #"bEI_thal":bEI_thal,
...
connectivityI_thal = OperatorTemplate(name="connectivityI_thal", 
                           equations= "m_out = (-gI_thal*(1-bEI_thal))*m_outC",
                           variables={
                            "m_out": "output", 
                            #"g_thal": -g_thal,
                            #"bEI_thal":bEI_thal,
...
# %%
# Node templates
cells_ext = cells + ['BACKGROUND'] + ['G'] + ['GE_THAL'] + ['BEI'] + ['GI_THAL'] + ['BEI_THAL']
nodes = [
    NodeTemplate(
        name=cells_ext[i],
        path=None,
        operators=(
            ...
            else [gE_thal_definition] if i == N_cells + 2 # GE_THAL
            else [bEI_definition] if i == N_cells + 3 # BEI
            else [gI_thal_definition] if i == N_cells + 4 # GI_THAL
            else [bEI_thal_definition] if i == N_cells + 5 # BEI_THAL
...
for i, cell_i in enumerate(cells):
    ...
    elif cell_i in ['thalE']:
        edges.append(('GE_THAL/gE_thal_definition/gE_thalC', f'{cell_i}/{connectivity_names[i]}/gE_thal', None, {'weight': 1.0})) # GE_THAL
        edges.append(('BEI_THAL/bEI_thal_definition/bEI_thalC', f'{cell_i}/{connectivity_names[i]}/bEI_thal', None, {'weight': 1.0})) # BEI_THAL
    else:
        edges.append(('GI_THAL/gI_thal_definition/gI_thalC', f'{cell_i}/{connectivity_names[i]}/gI_thal', None, {'weight': 1.0})) # GI_THAL
        edges.append(('BEI_THAL/bEI_thal_definition/bEI_thalC', f'{cell_i}/{connectivity_names[i]}/bEI_thal', None, {'weight': 1.0})) # BEI_THAL
...
# %%
circuit_to_yaml(model_gthal, "model_gthal.yaml")
...
results = model_gthal.run(simulation_time=simulation_time,
                  step_size=step_size,
