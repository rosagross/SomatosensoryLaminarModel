...
rpo_names_thal = ["RPO_" + cell for cell in cells[-2:]]
rpo_names_bI = ["RPO_bI_" + cell for cell in cells[:-2]]

rpo_names = rpo_names_bI + rpo_names_thal
...
# background input: all S1 populations
rpo_bI = OperatorTemplate(
    name='RPO_bI', path=None,
    equations=['d/dt * v = i',
               'd/dt * i = H/tau * (bI+m_in) - 2 * i/tau - v/tau^2'],
    variables={'v': 'output',
               'i': 'variable',
               'm_in': 'input',
...
# "normal" RPO for the thalamus (both E and I)
rpo_thal = OperatorTemplate(
    name='RPO', path=None,
    equations=['d/dt * v = i',
...
rpos_thal = [deepcopy(rpo_thal).update_template(name=rpo_names_thal[i], variables={"tau": tau_a1_thal[i]}) for i in range(len(rpo_names_thal))]

rpos = rpos_bI + rpos_thal
...
nodes = [
    NodeTemplate(
        name=cells[i],
        path=None,
        operators=[pros[i]] + rpos + (rpo_Iext_thalE+create_I_ext  if i == 13 else [])
    )
...
for i in cells:
    if i != 'thalE':
        ...
        sources = results[[f'V_{i}/{rpo_name_ext}' for rpo_name_ext in rpo_names_extended[:N_cells+1]]]
