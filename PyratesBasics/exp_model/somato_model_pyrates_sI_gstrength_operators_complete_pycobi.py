...
class SomatoModelPyrates():
    ...
    def connectivity_weights(self):
        ...
        # self.W = self.p.get_connectivity(self.gE, self.gI, self.gEthal, self.gIthal, self.thal_connect, self.extI_cellcounts, self.bI_cellcounts, self.thal_cellcounts, area='all') <--- no connectivity operators
    ...
    def create_pyrates_model(self):
        ...
        sI_definition = OperatorTemplate(
            name="sI_definition",
            equations="sIC = sI_input",
            variables={"sIC": "output", "sI_input": f"input({float(self.strength_I)})"}
        )       
        ...
        sI_thal_definition = OperatorTemplate(
            name="sI_thal_definition",
            equations="sI_thalC = sI_thal_input",
            variables={"sI_thalC": "output", "sI_thal_input": f"input({float(self.sI_thal)})"}
        )
        
        # Connectivity operators
        connectivityE = OperatorTemplate(name="connectivityE", 
                           equations= "m_out = g*m_outC",
                           variables={
                            "m_out": "output", 
                            #"g": g,
                            #"bEI":bEI,
        ...
        connectivityI = OperatorTemplate(name="connectivityI", 
                           equations= "m_out = (-g*sI)*m_outC",
                           variables={
                            "m_out": "output", 
                            "g": "input",
                            "sI": "input",
                            #"g": float(self.coupling_strength),
                            #"sI": float(self.strength_I),
                            #"connect_reverse_factor":f"input({float(connect_reverse_factor)})",
                            "m_outC":"input"
                            }
                            )
        connectivityE_thal = OperatorTemplate(name="connectivityE_thal", 
                           equations= "m_out = g_thal*m_outC",
                           variables={
                            "m_out": "output", 
                            "g_thal": "input",
                            #"bEI_thal":bEI_thal,
                            #"g_thal": float(self.g_thal),
                            #"bEI_thal": float(selfSS.bEI_thal),
        ...
        connectivityI_thal = OperatorTemplate(name="connectivityI_thal", 
                           equations= "m_out = (-g_thal*sI_thal)*m_outC",
                           variables={
                            "m_out": "output", 
                            "g_thal": "input",
                            "sI_thal": "input",
                            #"g_thal": float(self.g_thal),
                            #"bEI_thal": float(self.sI_thal),
                            #"connect_reverse_factor_thal":f"input({float(connect_reverse_factor)})",
                            "m_outC":"input"
                            }
                            )
        ...
        #  adding the connectivity operators definition nodes 
        # coupling_strength:
        nodes.append(
            NodeTemplate(
        name="G",
        path=None,
        operators=[g_definition]
            )
        )
        # sI
        nodes.append(
        ...
        for i, cell_i in enumerate(self.cells):
            if cell_i not in ("ThalE", "ThalI"):
                ...
                if i in self.idx_I:
                    edges.append(('SI/sI_definition/sIC', f'{cell_i}/{self.connectivity_names[i]}/sI', None, {'weight': 1.0})) # BEI
            else:
                edges.append(('G_THAL/g_thal_definition/g_thalC', f'{cell_i}/{self.connectivity_names[i]}/g_thal', None, {'weight': 1.0})) # G_THAL
                if cell_i == "ThalI":
                    edges.append(('SI_THAL/sI_thal_definition/sI_thalC', f'{cell_i}/{self.connectivity_names[i]}/sI_thal', None, {'weight': 1.0})) # BEI_THAL
        ...
        all_node_names = self.cells + ["background_input", "G", "SI", "G_THAL", "SI_THAL"]
