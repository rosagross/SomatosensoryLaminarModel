...
# Iext = A*sgn(sin(2pi*f*t))
# square_wave(t, omega, A, k=50) = A * tanh(k * sin(omega * t))
...
# thalE
rpo_thalE = [deepcopy(rpo).update_template(
    name=rpo_names[13], 
    equations=[#'Iext = A * (1/(1 + exp(-100*(t - t_onset))) * (1 - 1/(1 + exp(-100*(t - t_onset - f)))))',
                'Iext = A*sign(sin(2*pi*f*t))',
               'd/dt * v = i',
...
# thalI
rpo_thalI = [deepcopy(rpo).update_template(name=rpo_names[14], variables={"tau": tau_a1_thal[14],"bI":0.0})]

rpos = rpos_a1 + rpo_thalE + rpo_thalI
...
#circuit_to_yaml(area_1_thal_iext, "area_1_thal_iext.yaml")
