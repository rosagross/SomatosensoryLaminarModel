...
pro_layer_names = ["PRO_LAYER_"+ cell for cell in cells]
rpo_names=["RPO_"+cell for cell in cells]

# %% Operator template for the PRO
pro = OperatorTemplate(
    name='PRO', path=None,
    equations=["m_out_temp = m_max / (1 + exp(r*(V_thr - v)))"],
    variables={'m_out_temp': 'output',
               'v': 'input',
               'r': 0.1,
               'V_thr': 35.0,
               'm_max': 70.0},
...
# %% Operator template for the PRO layer
pros_layer = [OperatorTemplate(
    name=pro_layer_names[i], path=None,
    equations=[f"d/dt* m_out_sv_{i} = 0.0",
               f"m_out_sv_{i} = m_out_temp2",
               f"m_out = m_out_sv_{i}"],
    variables={'m_out_temp2': 'input',
               f'm_out_sv_{i}': 'variable',
               'm_out': 'output'
               },
    description="sigmoidal potential-to-rate operator (layer)") for i in range(N_cells)]
...
# %%
# Node templates
nodes = [NodeTemplate(name=cells[i], path=None, operators= [pros[i],pros_layer[i]] + rpos) for i in range(N_cells)]
...
for i, cell_i in enumerate(cells):
    edges.append((f'{cell_i}/{pro_names[i]}/m_out_temp', f'{cell_i}/{pro_layer_names[i]}/m_out_temp2', None, {'weight': 1.0}))
    # j : source
    for j, cell_j in enumerate(cells):
        edges.append((f'{cell_j}/{pro_layer_names[j]}/m_out', f'{cell_i}/{rpo_names[j]}/m_in', None, {'weight': W_A1[i,j]}))
...
for idx, source_cell in enumerate(cells):
    pro_layer_name = pro_layer_names[idx]  
    outputs_pro[f'm_{source_cell}'] = f'{source_cell}/{pro_layer_name}/m_out_sv_{idx}'
"""
