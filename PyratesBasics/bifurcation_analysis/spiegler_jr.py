"""Extended Jansen-Rit definition through Pyrates (python) and bifurcation analysis using PyCobi (see Spiegler, 2010)"""
# %%
...
# Extended Model definition --> Pyrates
# following Spiegler's notation, EIN = 1, IIN = 2, PC = 3

# %% Operators 
pro_pc = OperatorTemplate( # the sigmoid (PRO) remains the same of the original Jansen-Rit + INTRINSIC INPUTS v_iT
    name='PRO_pc', path=None,
    equations=["m_out = 2.*m_max / (1 + exp(r*(V_thr - (v+v_it))))"],
    variables={'m_out': 'output',
               ...
               'm_max': 2.5,
               'r': 560.0},
    description="sigmoidal potential-to-rate operator for the pyramidal cells")

pro_ein = deepcopy(pro_pc).update_template(
    name='PRO_ein', path=None, variables={'v_it': 0.0}, description="sigmoidal potential-to-rate operator for the EIN"
)

pro_iin = deepcopy(pro_pc).update_template(
    name='PRO_iin', path=None, variables={'v_it': 0.0}, description="sigmoidal potential-to-rate operator for the IIN"
)
...
"""
# extrinsic inputs: see the relationship between m and p + difference in the operators
rpo_e_in_pc = deepcopy(rpo_e).update_template(
    name='RPO_e_in_pc', 
    path=None, 
    equations=['d/dt * v = i',
               'd/dt * i = H/tau * (m_in+(c_it*m_it)) - 2 * i/tau - v/tau^2'], 
    variables={'m_it': 0.0, 'c_it': 0.0}, # CHANGE!
    description="excitatory rate-to-potential operator with extrinsic input for the PC population"
)

rpo_e_in_ein = deepcopy(rpo_e_in_pc).update_template(
    name='RPO_e_in_ein', path=None, variables={'m_it': 0.0, 'c_it': 0.0}
)

rpo_e_in_iin = deepcopy(rpo_e_in_pc).update_template(
    name='RPO_e_in_iin', path=None, variables={'m_it': 0.0, 'c_it': 0.0}
)
"""

# %% Nodes
ein = NodeTemplate(name="EIN", path=None, operators=[pro_ein, rpo_e])
iin = NodeTemplate(name="IIN", path=None, operators=[pro_iin, rpo_e])
pc = NodeTemplate(name="PC", path=None, operators=[pro_pc, rpo_e, rpo_i])

# %% Circuit
jrc_extended = CircuitTemplate(
    name="JRC", nodes={'PC': pc, 'EIN': ein, 'IIN': iin},
    edges=[("PC/PRO_pc/m_out", "IIN/RPO_e/m_in", None, {'weight': 33.75}),
           ("PC/PRO_pc/m_out", "EIN/RPO_e/m_in", None, {'weight': 135.}),
           ("EIN/PRO_ein/m_out", "PC/RPO_e/m_in", None, {'weight': 108.}),
           ("IIN/PRO_iin/m_out", "PC/RPO_i/m_in", None, {'weight': 33.75})],
    path=None)

# %% Definition of the system with PyCobi

auto_dir = "/data/u_mecozzi_software/miniforge3/envs/pyrates_project/auto-07p"
jrc_auto = ODESystem.from_template(jrc_extended, auto_dir=auto_dir, init_cont = False, NPR=100, NMX=30000)
...
# %% Bifurcation analysis on the parameter "v_3T" (starting from the initial equilibrium point [0,0,0,0,0,0])

v_3T_sols, v_3T_cont = jrc_auto.run(
    origin=t_cont, 
    starting_point='EP', 
    name='v_3T', # Name of continuation parameter (variable to vary) 
    bidirectional=True, #  Continue both forward and backward from starting point
    ICP=9, # Index of parameter(s) to continue in (AUTO parameter numbering) --> checl from the ODESystem!
    RL0=-2.0, RL1=10.0, # Lower and upper bounds for continuation parameter(s)
    ...
    THL={}, 
    THU={}, 
    UZR={'PC/PRO_pc/v_it': 10}, 
    STOP={'UZR'}
)
#%%
jrc_auto.plot_continuation('PAR(9)', 'U(1)', cont='v_3T')
plt.show()
...
y1 = v_3T_sols["PC/RPO_e/v"].to_numpy()*1000
y2 = -v_3T_sols["PC/RPO_i/v"].to_numpy()*1000
y = y1-y2
y0 = (A/a)*((2*e0)/(1+np.exp(r*(v0-y))))
p = v_3T_sols["PC/PRO_pc/v_it"]
