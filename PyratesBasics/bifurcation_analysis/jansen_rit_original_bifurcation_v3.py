...
# %% Model definition (for more detailed infos see "jansenrit_p.py")
...
# %% Time continuation with PyCobi (see "jansenrit_p.py" for the time integration with PyRates)
...
#%%
jrc_auto.plot_continuation('PAR(1)', 'U(7)', cont='u')
plt.show()
# %% 
# y1 = U(1)
# y2 = -U(3)
...
y01 = u_sols["EIN/RPO_e/v"].to_numpy()*1000
y02 = u_sols["IIN/RPO_e/v"].to_numpy()*1000
#y0 = y01 - y02
...
#y = v0-((1/r)* (np.log( ((2*A*e0)/(a*y0)) -1))  )


# %%
p = u_sols["PC/RPO_e_in/u"]
#plt.plot(p,y,label='y')
plt.plot(p,y0,label='y_0')
#plt.plot(p,y1,label='y_1')
#plt.plot(p,y2,label='y_2')
plt.ylabel('[mV]')
plt.xlabel('p [Hz]')
plt.legend()
plt.show()
